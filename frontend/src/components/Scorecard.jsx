...
const Scorecard = ({ interviewId, onBack }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setScorecard({
          ...
          technical: data.technical ?? 5,
          coding: data.coding ?? 5,
          attitude: data.attitude ?? 5,
  ...
  const handleSave = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5196/api/hr/scorecard/save`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-3">Overall HR Score (0–100)</h2>
          ...
          <input
            ...
            onChange={(e) => setScorecard({ ...scorecard, score: parseInt(e.target.value) || 0 })}
        ...
        {/* Notes */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">HR Notes / Observations</h2>
          {/* <textarea
            ...
            rows="6"
            placeholder="Add any notes regarding the candidate's performance, behavior, strengths, weaknesses, or observations..."
          /> */}
          <textarea
            value={scorecard.notes}
            onChange={(e) => setScorecard({ ...scorecard, notes: e.target.value })}
            ...
            rows="6"
            placeholder="Notes imported from Meeting Room..."
          />
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <textarea
            ...
            rows="6"
            placeholder="Any additional comments or notes about the candidate..."
          />
        </div>
      </div>
    </div>
