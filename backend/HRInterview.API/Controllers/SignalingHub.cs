...
//new
// Controllers/SignalingHub.cs - UPDATED WITH LARGE MESSAGE SUPPORT
using System.Collections.Concurrent;
using System.Net.WebSockets;
using System.Text;
            ...
            foreach (var recipient in recipients)
            {
                if (recipient.Value.State == WebSocketState.Open)
                {
                    try
                    {
                        ...
                        // Send large messages in chunks if needed
                        const int chunkSize = 1024 * 32; // 32KB chunks
                        
                        if (bytes.Length <= chunkSize)
                        {
                            // Send small messages in one go
                            await recipient.Value.SendAsync(
                                new ArraySegment<byte>(bytes),
                                WebSocketMessageType.Text,
                                true,
                        ...
                        {
                            // Send large messages (files) in chunks
                            for (int i = 0; i < bytes.Length; i += chunkSize)
                            {
                                var count = Math.Min(chunkSize, bytes.Length - i);
                                var isLastChunk = (i + count) >= bytes.Length;
                                
                                await recipient.Value.SendAsync(
                                    new ArraySegment<byte>(bytes, i, count),
                                    WebSocketMessageType.Text,
                                    isLastChunk,
                                    CancellationToken.None
                                );
                            }
                            Console.WriteLine($"[SignalingHub] 📎 Sent large message ({bytes.Length} bytes) to {recipient.Key}");
                        }
                    }
                    catch (Exception ex)
                    {
